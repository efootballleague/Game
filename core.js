...
function goPage(name) {
  ...
  if (name === 'pm') {
    // DMs live inside the chat/messenger page now — redirect there
    goPage('chat');
    return;
  }
  if (name === 'chat') {
...
function startListeners(authTimeout, onResolved) {
  ...
  auth.onAuthStateChanged(function (user) {
    ...
    if (user) {
      // Admin bypasses profile check — enters app directly
      if (user.email === ADMIN_EMAIL) {
        myProfile = { uid:user.uid, username:'Admin', email:user.email,
          league:'epl', club:'', avatar:'', banned:false, isAdmin:true };
        me = user;
        enterApp();
      ...
      db.ref(DB.players + '/' + user.uid).once('value', function (s) {
        ...
        listenNotifs(); // start in-app notification banners
        if (typeof listenRedDots === 'function')  listenRedDots();
...
// listenNotifs is defined in features.js — stub removed to avoid conflict
var _notifOff = null;
...
// renderNotifications is defined in features.js
