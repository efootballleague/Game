...
// ── POLLS ─────────────────────────────────────────────────────
// Full implementation below at line ~563 (renderPolls override)
// castVote is standalone
...
function closePoll(key)  { if(!db)return; db.ref(DB.polls+'/'+key+'/active').set(false).then(function(){toast('Poll closed.');renderPolls();}); }
function reopenPoll(key) { if(!db)return; db.ref(DB.polls+'/'+key+'/active').set(true).then(function(){toast('Poll reopened.');renderPolls();}); }
function deletePoll(key) { if(!confirm('Delete this poll?'))return; if(!db)return; db.ref(DB.polls+'/'+key).remove().then(function(){toast('Deleted.');renderPolls();}); }
