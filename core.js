// ============================================================
// CORE.JS — Helpers, Navigation, Firebase Boot
// Clean rebuild — all fixes integrated
// ============================================================
...
function toast(msg,type) {
  ...
  e.style.opacity='1'; e.style.transform='translateY(0)'; e.style.display='block';
  clearTimeout(e._t);
...
function clubColor(name) {
  ...
  for(var i=0;i<lids.length;i++){var c=getClub(lids[i],name);if(c&&c.color&&c.color!=='#888')return c.color;}
...
function clubBadge(name,lid,sz) {
  ...
  if(club.logo){
    return '<div style="'+style+'" title="'+esc(name)+'">'+init
      ...
      +'onload="this.style.opacity=1" onerror="this.remove()"></div>';
  }
...
function enterApp() {
  ...
  renderHomeStats(); renderRecentRes(); renderTopPlayers();
...
function goPage(name) {
  if(_navLock) return;
  _navLock=true; setTimeout(function(){_navLock=false;},250);
  hideLanding();
  document.querySelectorAll('.page').forEach(function(p){p.classList.remove('active');});
  document.querySelectorAll('.nb').forEach(function(b){b.classList.remove('active');});
  window.scrollTo(0,0);
  ...
  if(name==='predict')     { if(typeof renderPredictions==='function') renderPredictions(); }
  if(name==='notifications'){ if(typeof renderNotifications==='function') renderNotifications(); }
  if(name==='friendly')    { if(typeof renderFriendlyPage==='function') renderFriendlyPage(); }
  if(name==='pm')          { goPage('chat'); return; }
  if(name==='chat') {
    ...
    if(myProfile){if(cr)cr.classList.remove('hidden');if(cl)cl.classList.add('hidden');}
    else{if(cr)cr.classList.add('hidden');if(cl)cl.classList.remove('hidden');}
    setTimeout(function(){var b=$('chat-msgs');if(b)b.scrollTop=b.scrollHeight;},200);
  }
  // Chat FAB — hide on chat page
  var fab=$('chat-fab');
  if(fab) fab.style.display=(name==='chat')?'none':(myProfile?'flex':'none');
}
...
function updateNav() {
  ...
  if(myProfile&&myProfile.username) {
    var club=getClub(myProfile.league,myProfile.club);
    var init=myProfile.username.charAt(0).toUpperCase();
    e.innerHTML='<button class="topbar-club-btn" onclick="goPage(\'profile\')" title="'+esc(myProfile.username)+'">'
      +(club&&club.logo?'<img src="'+club.logo+'" onerror="this.outerHTML=\''+init+'\'">':init)
      +'</button>'
    ...
    var fab=$('chat-fab');          if(fab) fab.style.display='flex';
  } else {
...
function openDrawer(){
  ...
  if(d)d.classList.add('open');if(o)o.classList.add('active');if(h)h.classList.add('open');
  document.body.style.overflow='hidden';
}
function closeDrawer(){
  var d=$('drawer'),o=$('drawer-overlay'),h=$('hamburger');
  if(d)d.classList.remove('open');if(o)o.classList.remove('active');if(h)h.classList.remove('open');
...
function updateDrawer(){
  ...
  if(myProfile&&myProfile.username){
    ...
    if(dav){
      if(myProfile.avatar&&myProfile.avatar.length<10){dav.textContent=myProfile.avatar;dav.style.fontSize='1.4rem';}
      else if(myProfile.avatar&&myProfile.avatar.startsWith('http')){dav.innerHTML='<img src="'+myProfile.avatar+'" style="width:100%;height:100%;object-fit:cover;border-radius:50%">';}
      else{dav.textContent=(myProfile.username||'?').charAt(0).toUpperCase();}
    }
  } else {
...
// ── MISC ──────────────────────────────────────────────────────
window.addEventListener('scroll',function(){var tb=$('topbar');if(tb)tb.classList.toggle('scrolled',window.scrollY>8);},{passive:true});
document.addEventListener('visibilitychange',function(){if(document.visibilityState==='visible'&&db)setTimeout(debouncedRefresh,300);});
...
function isRestricted(uid,type){
  var p=allPlayers[uid]||myProfile; if(!p||!p.restrictions)return false;
...
function sendNotif(toUID,payload){
  ...
  db.ref(DB.notifs+'/'+toUID).push({
    title:payload.title||'eFootball Universe',body:payload.body||'',
    icon:payload.icon||'⚽',link:payload.link||'',type:payload.type||'info',
...
function launchEfootball(){
  ...
  if(/iPhone|iPad|iPod/.test(ua)){
    var iframe=document.createElement('iframe');iframe.style.display='none';iframe.src='efootball://';
    document.body.appendChild(iframe);
    setTimeout(function(){document.body.removeChild(iframe);window.location.href='https://apps.apple.com/app/efootball-2024/id1448787484';},2500);
  }else if(/Android/.test(ua)){window.open('https://play.google.com/store/apps/details?id=com.konami.efootball.android','_blank');}
  else{openMo('efootball-links-mo');}
}
...
function initRefereeSystem()     { if(typeof listenRefDuties==='function')listenRefDuties(); if(typeof checkPendingAutoApprovals==='function')checkPendingAutoApprovals(); }
...
function initApp(){
  showLoader();
  (function(){var s=document.createElement('script');s.src='https://js.paystack.co/v2/inline.js';s.async=true;document.head.appendChild(s);})();
  if(!firebase.apps.length) firebase.initializeApp(FIREBASE_CONFIG);
  auth=firebase.auth(); db=firebase.database();
  var authResolved=false;
  var authTimeout=setTimeout(function(){
    if(!authResolved){console.warn('Auth timeout');authResolved=true;enterApp();}
  },6000);
...
function startListeners(authTimeout,onResolved){
  db.ref(DB.players).on('value',function(s){allPlayers=s.val()||{};debouncedRefresh();});
  db.ref(DB.matches).on('value',function(s){
    allMatches=s.val()||{};debouncedRefresh();
    var pg=activePage();
  ...
  setTimeout(function(){
    db.ref(DB.online).on('value',function(s){var e=$('online-count');if(e)e.textContent=Object.keys(s.val()||{}).length;});
    db.ref(DB.penalties).on('value',function(s){allPenalties=s.val()||{};if(activePage()==='leagues')renderStd(curLg);});
    db.ref(DB.polls).on('value',function(s){allPolls=s.val()||{};if(activePage()==='polls')renderPolls();renderPollBadge();});
    db.ref(DB.uclSet).on('value',function(s){uclSettings=s.val()||{};if(activePage()==='ucl')renderUCL();});
    db.ref(DB.uclPay).on('value',function(s){uclPayments=s.val()||{};if(activePage()==='ucl')renderUCL();});
  ...
  auth.onAuthStateChanged(function(user){
    ...
    if(user){
      if(user.email===ADMIN_EMAIL){
        myProfile={uid:user.uid,username:'Admin',email:user.email,league:'epl',club:'',avatar:'',banned:false,isAdmin:true};
        me=user; enterApp();
        setOnline();listenUnread();listenGlobalDMs();
        initRefereeSystem();listenMatchRooms();initSwap();
        if(typeof listenNotifs==='function')listenNotifs();
      ...
      db.ref(DB.players+'/'+user.uid).once('value',function(s){
        ...
        if(!data||!data.username){
          ...
          if(user.providerData&&user.providerData[0]&&user.providerData[0].providerId==='google.com'){
            if(typeof openGoogleSetup==='function')openGoogleSetup(user);else showLanding();
          }else{showLanding();}
          return;
        }
        myProfile=data; enterApp();
        db.ref(DB.players+'/'+user.uid).on('value',function(s2){var d2=s2.val();if(d2&&d2.username)myProfile=d2;updateNav();updateDrawer();});
        setOnline();listenUnread();listenGlobalDMs();
        initRefereeSystem();listenMatchRooms();initSwap();
    ...
    }else{myProfile=null;me=null;enterApp();}
  },function(err){console.error('Auth error:',err);if(authTimeout)clearTimeout(authTimeout);enterApp();});
}
...
function initPushNotifications(){
  ...
  Notification.requestPermission().then(function(p){if(p==='granted')db.ref(DB.players+'/'+me.uid+'/notifPermission').set(true);});
}
