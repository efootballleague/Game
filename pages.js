...
function openPrepModal(mid) {
  ...
  // Pre-fill time from existing match so user doesn't have to re-enter it
  var timeInp = $('prep-time');
  if (timeInp) {
    if (m && m.matchTime) {
      var pad = function(n){ return n<10?'0'+n:n; };
      var dt = new Date(m.matchTime);
      timeInp.value = dt.getFullYear()+'-'+pad(dt.getMonth()+1)+'-'+pad(dt.getDate())
...
function submitPrep() {
  ...
  // Time is optional — keep existing match time if not provided
  var newTime = time ? new Date(time).getTime() : (m && m.matchTime) || Date.now();

  var updates = { roomCode:code, prepNote:note, prepSetAt:Date.now() };
  if (time) updates.matchTime = newTime; // only update time if user changed it

  db.ref(DB.matches + '/' + mid).update(updates)
    .then(function () {
      closeMo('prep-mo');
      toast('Room code dropped! Away team notified.');
      if (ap && myProfile) {
        var dk = dmKey(myProfile.uid, ap.uid);
        var timeStr = fmtFull(newTime);
        db.ref(DB.dm + '/' + dk).push({
          from:myProfile.uid, fromName:myProfile.username,
          text:'🏟️ Room Code: ' + code + ' | Time: ' + timeStr + (note ? ' | ' + note : ''),
          ts:Date.now(), system:true
        });
        db.ref(DB.dmMeta + '/' + dk).update({
          lastMsg:'Code: ' + code, lastTs:Date.now(),
          ['participants/' + myProfile.uid]:true,
          ['participants/' + ap.uid]:true
        });
        db.ref(DB.dmUnread + '/' + ap.uid + '/' + dk).transaction(function(v){ return (v||0)+1; });
        sendNotif(ap.uid, { title:'🏟️ Room Code Ready!', body: myProfile.username + ' dropped the code: ' + code, type:'room_code', code:code, matchId:mid });
      }
    })
