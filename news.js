...
function generateAllNews(){
  ...
  var _pg=activePage();
  if(_pg==='home'||_pg==='news') renderNewsAnchor();
...
function genCross(){
  ...
  // ── NEW SEASON + MATCHDAY + CUP STORIES ─────────────────────
  items = items.concat(genSeasonStories());
  items = items.concat(genCupNews());
...
var TYPE_ICON={title:'🏆',form:'🔥',crisis:'🚨',podium:'🎯',bottomFight:'📉',penalty:'⚡',cleansheet:'🧤',goals:'⚽',unbeaten:'🛡',result:'📊',upcoming:'📅',activity:'💪',discipline:'⚠',milestone:'🎉',community:'👥',season:'🚀',matchday:'📋',schedule:'📅',friendly:'⚔️',friendlyResult:'⚔️',cup:'🥇',cupDraw:'🎱',cupResult:'🥇'};
var TYPE_COLOR={title:'#FFE600',form:'#00FF85',crisis:'#FF2882',podium:'#00D4FF',bottomFight:'#FF8C00',penalty:'#FF2882',cleansheet:'#00D4FF',goals:'#FF8C00',unbeaten:'#00FF85',result:'#fff',upcoming:'#FFE600',activity:'#00D4FF',discipline:'#FF2882',milestone:'#FFE600',community:'#00D4FF',season:'#00FF85',matchday:'#00D4FF',schedule:'#FFE600',friendly:'#FF2882',friendlyResult:'#FF2882',cup:'#FFE600',cupDraw:'#00D4FF',cupResult:'#FFE600'};
...
function genCupNews() {
  var items = [];
  if (typeof CUPS === 'undefined') return items;
  Object.keys(CUPS).forEach(function(cid) {
    var cup = CUPS[cid];
    // Check if there are cup fixtures in allMatches
    var cupMs = Object.values(allMatches).filter(function(m){ return m.cupId===cid; });
    if (!cupMs.length) return;
    var played = cupMs.filter(function(m){ return m.played; });
    var upcoming = cupMs.filter(function(m){ return !m.played && m.homeId && m.awayId; });
    var round = cup.rounds && cup.rounds[0] || 'Round 1';

    if (upcoming.length) {
      var seed = cid + 'cup' + upcoming.length;
      var pairings = upcoming.slice(0,4).map(function(m){
        var hp=allPlayers[m.homeId], ap=allPlayers[m.awayId];
        if(!hp||!ap) return null;
        return hp.username + ' vs ' + ap.username;
      }).filter(Boolean).join(' · ');
      items.push(makeStory('all','cup',88,
        '🥇 ' + cup.n + ' — ' + round + ' draw confirmed',
        upcoming.length + ' fixture' + (upcoming.length>1?'s':'') + ' confirmed for ' + cup.short + ' ' + round + '.',
        [cup.n + ' ' + round + ' has been drawn. ' + upcoming.length + ' fixture' + (upcoming.length>1?'s':'') + ' confirmed.',
         pairings ? 'The matchups: ' + pairings + '.' : 'All fixtures are locked in.',
         'No league points — only silverware on the line. Every game is knockout. One loss and you are out.',
         'The cup run begins. Here we go! ✅'],
        {}));
    }

    if (played.length) {
      var lastPlayed = played[played.length-1];
      var hp=allPlayers[lastPlayed.homeId], ap=allPlayers[lastPlayed.awayId];
      if (hp&&ap) {
        var win = lastPlayed.hg>lastPlayed.ag?hp : lastPlayed.ag>lastPlayed.hg?ap:null;
        var sub = win ? win.username+' wins '+lastPlayed.hg+'-'+lastPlayed.ag+' in the '+cup.short+'.'
          : hp.username+' and '+ap.username+' level in the '+cup.short+' — '+lastPlayed.hg+'-'+lastPlayed.ag+'.';
        items.push(makeStory('all','cupResult',80,
          '🥇 '+cup.short+' result: '+hp.username+' '+lastPlayed.hg+'-'+lastPlayed.ag+' '+ap.username,
          sub,
          [sub,
           win ? win.username+' advances. '+( win===hp?ap.username:hp.username)+' is eliminated. No second chances in the cup.'
               : 'Replays may be needed. Cup football at its most dramatic.',
           'Here we go! ✅'],
          {homeId:lastPlayed.homeId,awayId:lastPlayed.awayId,homeName:hp.username,awayName:ap.username}));
      }
    }
  });
  return items;
...
function genSeasonStories() {
  ...
  var scheduled = allMs.filter(function(m){ return !m.played && m.homeId && m.awayId && m.matchTime && !m.cupId; });
  var playedCount = allMs.filter(function(m){ return m.played && !m.cupId; }).length;
  ...
  function seedNum(str){ var h=0; for(var i=0;i<str.length;i++) h=(h*31+str.charCodeAt(i))&0x7FFFFFFF; return h; }
  ...
  if (playedCount === 0 && totalScheduled >= 4) {
    var skVars = [
      { sub: 'The new season is officially live. '+totalScheduled+' fixtures confirmed across all leagues.',
        body: ['eFootball Universe is BACK. The fixtures are set — '+tp+' managers ready to fight for glory.',totalScheduled+' matches scheduled across all four leagues. Every matchday loaded.',
               'From Match Day 1, this season will produce drama, upsets, comebacks, and champions.','New season, new chances. Here we go! ✅'] },
      { sub: tp+' managers. '+totalScheduled+' fixtures. One season to decide everything.',
        body: ['The wait is over. eFootball Universe is LIVE — fixtures confirmed, managers ready.',
               'Every title race begins with Match Day 1. That day is here.','The leagues are competitive. Nobody is giving anything away.',
               'What a season this will be. Here we go! ✅'] },
      { sub: 'Fixtures confirmed. The new eFootball Universe season begins now.',
        body: ['IT IS HERE. '+totalScheduled+' fixtures confirmed, '+tp+' managers locked in.',
               'EPL, La Liga, Serie A, Ligue 1 — all four leagues ready. Title races begin immediately.',
               'Every result counts. Every point could be the difference.','The stage is set. Here we go! ✅'] },
      { sub: 'Season confirmed — '+totalScheduled+' fixtures, '+tp+' managers. Let the competition begin.',
        body: ['eFootball Universe has a new season — '+totalScheduled+' fixtures across all leagues.',
               'The schedule is out. Every manager now knows their path — and their opponents.',
               'Title contenders, dark horses — Match Day 1 will tell us who is who.','Here we go! ✅'] },
      { sub: tp+' managers enter the arena. The fixtures are out. Here We Go.',
        body: ['Months of anticipation ends today. '+totalScheduled+' fixtures confirmed across four leagues.',
               'Who is the favourite? Who is the dark horse? Match Day 1 gives the first answers.',
               'The most competitive season yet. Here we go! ✅'] },
    ];
    var sv = skVars[seedNum('season'+totalScheduled+tp) % skVars.length];
    items.push(makeStory('all','season',99,'🚀 NEW SEASON LIVE — '+tp+' managers, '+totalScheduled+' fixtures!',sv.sub,sv.body));
  }

  leagueIds.forEach(function(lid) {
    var lg = LGS[lid]||{};
    var md1 = allMs.filter(function(m){ return m.league===lid&&m.matchDay===1&&!m.played&&m.homeId&&m.awayId&&!m.cupId; });
    ...
    var md1Vars = [
      { sub:'Match Day 1 confirmed for '+lg.n+'. '+md1.length+' opener'+(md1.length>1?'s':'')+' to launch the season.',
        body:[lg.n+' kicks off with Match Day 1. '+md1.length+' fixture'+(md1.length>1?'s':'')+' set the tone for the entire season.',
              'Opening day: '+pairings.join(' · ')+'.',
              'First impressions matter. Whoever wins Match Day 1 sends a message to the rest of the league.',
              'It starts here. '+lg.n+' is open for business. Here we go! ✅'] },
      { sub:lg.n+' Match Day 1: '+md1.length+' match'+(md1.length>1?'es':'')+' confirmed, kicking off '+timeStr+'.',
        body:['The '+lg.n+' season opener is confirmed. Match Day 1 is here.','Opening matchups: '+pairings.join(' · ')+'.',
              'No form, no momentum — just raw desire. Match Day 1 is the great equaliser.','Here we go! ✅'] },
      { sub:'All eyes on '+lg.n+' Match Day 1. The season officially begins.',
        body:[lg.n+' Match Day 1 is ready. '+md1.length+' fixture'+(md1.length>1?'s':'')+' confirmed.','On the card: '+pairings.join(' · ')+'.',
              'Tight title race? Dominant champion? Match Day 1 gives the first clues.','The opening whistle is everything. Here we go! ✅'] },
    ];
    var mv = md1Vars[seedNum(lid+'md1'+md1.length+pairings.length) % md1Vars.length];
    items.push(makeStory(lid,'matchday',97,'📋 '+lg.n+' Match Day 1: Season opener confirmed',mv.sub,mv.body));
  });

  leagueIds.forEach(function(lid) {
    var lg=LGS[lid]||{};
    var unplayedMDs = {};
    allMs.filter(function(m){ return m.league===lid&&!m.played&&m.matchDay&&m.homeId&&m.awayId&&!m.cupId; })
    ...
    var mdpVars = [
      { sub:lg.n+' Match Day '+nextMD+' is next. '+fixtures.length+' fixture'+(fixtures.length>1?'s':'')+' confirmed.',
        body:['After the opening rounds, '+lg.n+' reaches Match Day '+nextMD+'. Stakes are rising.',
              'On the card: '+pairings.join(' · ')+'.','The table is taking shape. Every point matters now.',
              'Match Day '+nextMD+' of '+totalMDs+'. Here we go! ✅'] },
      { sub:'Match Day '+nextMD+' preview: '+pairings.join(' · ')+'.',
        body:[lg.n+' continues with Match Day '+nextMD+'. The pressure is building.',
              pairings.length+' match'+(pairings.length>1?'es':'')+': '+pairings.join(' · ')+'.',
              'Leaders want to extend. The rest want to close the gap.','Watch closely. Here we go! ✅'] },
    ];
    var mpv = mdpVars[seedNum(lid+'mdp'+nextMD+fixtures.length) % mdpVars.length];
    items.push(makeStory(lid,'matchday',Math.max(70,94-nextMD*2),'📋 '+lg.n+' Match Day '+nextMD+' preview',mpv.sub,mpv.body));
  });

  if (playedCount===0 && totalScheduled>=4) {
    leagueIds.forEach(function(lid) {
      var lg=LGS[lid]||{};
      var lgMs=allMs.filter(function(m){return m.league===lid&&m.matchDay&&!m.cupId;});
      ...
      var schVars = [
        { sub:pCount+' managers. '+lgMs.length+' fixtures across '+mdCount+' match days. One champion.',
          body:['The '+lg.n+' season is fully scheduled. '+pCount+' managers, '+lgMs.length+' fixtures.',
                'Home and away — every manager faces every other twice. No easy games in '+lg.n+'.',
                'One champion will emerge. But right now everyone starts equal.','Here we go! ✅'] },
        { sub:lg.n+' — '+mdCount+' match days, '+lgMs.length+' fixtures. Full season confirmed.',
          body:['The '+lg.n+' calendar is confirmed. '+mdCount+' match days, '+lgMs.length+' total fixtures.',
                'Every manager knows their games. The path to the title is set.',
                'The season waits for no one. Here we go! ✅'] },
      ];
      var sv2=schVars[seedNum(lid+'sch'+mdCount+pCount) % schVars.length];
      items.push(makeStory(lid,'schedule',88,'📅 '+lg.n+' — Full season fixture list confirmed',sv2.sub,sv2.body));
    });
  }
  return items;
}
